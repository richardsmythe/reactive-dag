...
<pre><code>
   ...
   async Task Print()
   {
       var s1 = await dag.GetResult<double[]>(state1);
       var s2 = await dag.GetResult<double[]>(state2);
       Console.WriteLine($"Day 1: {string.Join(", ", s1.Select((p, i) => $"{weather[i]}: {p:P2}"))}, Most likely: {await dag.GetResult<string>(day1Weather)}, Rain: {await dag.GetResult<double>(rain1):P2}");
       Console.WriteLine($"Day 2: {string.Join(", ", s2.Select((p, i) => $"{weather[i]}: {p:P2}"))}, Most likely: {await dag.GetResult<string>(day2Weather)}, Rain: {await dag.GetResult<double>(rain2):P2}");
   }

   Console.WriteLine("--- Initial ---");
   await Print();

   // Reactively update initial state
   Console.WriteLine("\n--- Now set initial state to 100% Rainy ---");
   await dag.UpdateInput(state0, new double[] { 0, 0, 1 });
   await Print();
</code></pre>
